...
const alertVariants = ['default', 'danger', 'success', 'warning'] as const;

const alertVariantsFn = cva(
...
function Alert({
  ...
  variant,
  ...props
}: React.ComponentProps<'div'> & VariantProps<typeof alertVariantsFn>) {
  return (
    <div
      data-slot='alert'
      role='alert'
      className={cn(alertVariantsFn({ variant }), className)}
      {...props}
    />
  );
}
