  ...
  const handleSelect = (newDay: DateRange | undefined) => {
    ...
    if (newStart) {
      if (!dateRange.from) {
        ...
        const newStartDate = add(dateRange.from ?? new Date(), {
    ...
    if (newEnd) {
      if (!dateRange.to) {
        ...
        const diffInDays = diff / (1000 * 60 * 60 * 24);
        const newEndDate = add(dateRange.to ?? new Date(), {
          days: Math.ceil(diffInDays),
        });
    ...
    return (
      <Drawer>
        <DrawerTrigger asChild onClick={readOnly ? (e) => e.preventDefault() : undefined}>
          <Button
            id={`${id}-date-time-range-picker-date`}
            variant={'outline'}
            className={cn(
            ...
            title={renderPickerText({
              formatString,
              defaultPlaceholder: getTranslation(
                'base.components.ui.DateTimeRangePicker.placeholder'
              ),
              locale,
              dateRange,
              placeholder,
            ...
            <div className='w-full space-y-2 p-4'>
              {toggleTime && (
                <div className='flex items-center gap-4'>
                  <Checkbox
                    id={`${id}-date-time-range-picker-show-time`}
                    checked={includeTime}
                  ...
                  />
                  <Label htmlFor='show-time'>Include time</Label>
                </div>
              )}
              {(!toggleTime || includeTime) && (
                <div className='flex flex-col items-center justify-center gap-2'>
                  <div className='flex flex-col items-center gap-2'>
                    {dateRange?.from && (
                      <span className='text-sm'>
                        From {format(dateRange.from, timeFormatString)} at
                      </span>
                    )}
                    <TimeInput
                  ...
                  <div className='flex flex-col items-center gap-2'>
                    {dateRange?.to && (
                      <span className='text-sm'>
                        To {format(dateRange.to, timeFormatString)} at
                      </span>
                    )}
                    <TimeInput
        ...
        <div className='w-full space-y-2 p-4'>
          ...
          {(!toggleTime || includeTime) && (
            <div className='flex items-center justify-center gap-4'>
              ...
              <div className='flex flex-col items-center gap-2'>
                {dateRange?.to && (
                  <span className='text-sm'>To {format(dateRange.to, timeFormatString)} at</span>
                )}
                <TimeInput
                  key={toTimeInputVersion}
                  date={dateRange?.to}
